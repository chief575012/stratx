if game.PlaceId ~= 3260590327 then 
    ...
    local TextGameWave = LocalPlayer.PlayerGui:WaitForChild("ReactGameTopGameDisplay"):WaitForChild("Frame"):WaitForChild("wave"):WaitForChild("container"):WaitForChild("value") -- currennt wave you are on
    ...
    function GetTimer()
        ...
        return {tonumber(GetCurrentWave()), Min, Sec + Recorder.SecondMili, tostring(TimerCheck)}
    end
    ...
    local GenerateFunction = {
        ...
        Skip = function(Args, Timer, RemoteCheck)
            if tonumber(GetCurrentWave()) == 0 then
                return
            end
    ...
    task.spawn(function()
        TextGameWave:GetPropertyChangedSignal("Text"):Wait()
        ...
        TextGameWave:GetPropertyChangedSignal("Text"):Connect(function()
            if tonumber(GetCurrentWave()) == FinalWave then
    ...
    appendstrat("local TDS = loadstring(game:HttpGet(\"https://raw.githubusercontent.com/chief575012/stratx/refs/heads/main/TDS/MainSource.lua\", true))()\nTDS:Map(\""..
